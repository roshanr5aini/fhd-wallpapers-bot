...
def available_video_qualities(info):
    """
    Return the standard quality buttons that have an actual source
    representation at or above that height, capped at 1080p.

    A requested quality is shown only if the source can provide that
    resolution without inventing/upscaling it.
    """
    heights = set()
    ...
    for quality in VIDEO_QUALITIES:
        if any(height >= quality for height in heights):
            available.append(quality)

    return available


def video_format_for_quality(quality):
    """
    Prefer an actual video stream at the requested height, otherwise the
    nearest lower stream. The quality buttons themselves are only shown
    when the source has enough resolution.
    """
    return (
...
def download_video(url, quality, job_dir):
    """
    Download a single video, merge audio/video with FFmpeg when needed,
    ...
    options = {
        "format": video_format_for_quality(quality),
...
def video_quality_keyboard(qualities):
    ...
    for quality in qualities:
        label = f"🔥 {quality}p" if quality >= 720 else f"📺 {quality}p"
        row.append(
            InlineKeyboardButton(
                label,
                callback_data=f"video_q_{quality}",
            )
        )
...
def video_info_text(info, qualities):
    ...
    return (
        ...
        f"{quality_text}\n\n"
        "⚠️ Maximum allowed duration: 5 minutes\n"
        "⚠️ Maximum quality: 1080p"
    )
...
async def handle_video_link(update, context, url):
    ...
    try:
        ...
        qualities = available_video_qualities(info)

        if not qualities:
            await status.edit_text(
                "❌ No supported video quality between 144p and 1080p "
        ...
        await status.edit_text(
            video_info_text(info, qualities),
            reply_markup=video_quality_keyboard(qualities),
        )

    except Exception as e:
        print("Video metadata error:", e)

        USER_VIDEO_SESSIONS.pop(user_id, None)

        await status.edit_text(
            "❌ I couldn't read this video link.\n\n"
            "Please check the link and try again."
        )


async def process_video_quality(update, context, quality):
    query = update.callback_query
    user_id = query.from_user.id
    chat_id = query.message.chat_id

    session = USER_VIDEO_SESSIONS.get(user_id)

    if not session:
        await query.message.edit_text(
            "⚠️ This video session has expired.\n\n"
            "Please send the video link again."
        )
        return

    if quality not in session["qualities"]:
        await query.message.edit_text(
            "❌ That quality is not available for this video."
        )
        return
    ...
    try:
        ...
        print("Video download/convert error:", e)

        await query.message.edit_text(
            "❌ Video download/conversion failed.\n\n"
            "Possible reasons:\n"
            "• The selected quality is unavailable.\n"
            "• The video is too large.\n"
            "• The platform blocked the download.\n"
            "• The link is no longer valid.\n\n"
            "Please try another video or quality."
        )

    finally:
        if not success and job_dir.exists():
            shutil.rmtree(job_dir, ignore_errors=True)
...
async def show_main_menu(update, context, edit=False):
    ...
    text = (
        ...
        "🎬 Video Link → Video\n"
        "Download a video link up to 5 minutes and choose 144p–1080p."
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "video_mode":
        ...
        await query.message.edit_text(
            ...
            "📺 Quality options: 144p → 1080p\n\n"
            "I will show only the qualities actually "
            "available in the source video.\n\n"
    ...
    if query.data.startswith("video_q_"):
        try:
            quality = int(query.data.split("_")[-1])
        except ValueError:
            await query.message.reply_text("❌ Invalid video quality.")
            return

        await process_video_quality(update, context, quality)
        return
